...
begin
  if Application.MessageBox('Se você continuar todos os dados inseridos a partir da' +#13+ 'data em que foi realizada a copia serão perdidos!' +#13+ 'Deseja continuar assim mesmo?','Cuidado!',MB_YESNO + MB_ICONWARNING) = mrYes then
  if ODLResBack.Execute then
    ...
    Application.MessageBox('A restauração da cópia de segurança foi realizada com sucesso!' +#13+ 'O aplicativo será finalizado para efetivação das auterações','',MB_OK + MB_ICONINFORMATION);
   ...
   begin
     ...
     if fileexists(copy(linha,pos('=',linha)+1,length(linha)))or(copy(linha,pos('=',linha)+1,length(linha))='0') then
       begin
        ...
        Application.MessageBox('Arquivo de Imagem de Fundo não Encontrado','Atenção',MB_OK+MB_ICONEXCLAMATION);
        break;
   end;
   closefile(arq);
...
begin
  ...
  frmsobre.ShowModal;}
  ShellAbout(Handle,'Folha de Pagamento','Versão 2.1 Desenvolvido por Denis em Delphi 7'#13'Trabalho de Conclusão de Curso',Application.Icon.Handle);
end;
